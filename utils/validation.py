...
def validate(hp, args, generator, discriminator, valloader, writer, step):
    ...
    for mel, audio in loader:
        ...
        for (feats_fake, score_fake), (feats_real, score_real) in zip(disc_fake, disc_real):
            loss_g += torch.mean(torch.sum(torch.pow(score_fake -
                                                     1.0, 2), dim=[1, 2]))
            for feat_f, feat_r in zip(feats_fake, feats_real):
                loss_g += hp.model.feat_match * \
                    torch.mean(torch.abs(feat_f - feat_r))
            loss_d += torch.mean(torch.sum(torch.pow(score_real -
                                                     1.0, 2), dim=[1, 2]))
            loss_d += torch.mean(torch.sum(torch.pow(score_fake,
                                                     2), dim=[1, 2]))
    ...
    writer.log_validation(loss_g_avg, loss_d_avg, generator,
